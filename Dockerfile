...
# size
RUN apk add --no-cache --virtual .python-db postgresql-libs postgresql-dev libffi-dev mariadb-dev

# Install runit and runit associated files
RUN apk add --no-cache runit

# runit-docker - painlessly use Runit in Docker containers
RUN wget -O runit-docker.tar.gz https://github.com/pixers/runit-docker/archive/1.1.tar.gz && \
...
RUN chmod +x /runit_bootstrap

# Copy over main NGINX config file
COPY nginx.conf /etc/nginx/nginx.conf

# Copy over NGINX custom config file and upload config files
...
RUN mkdir -p /etc/sv/waitress

COPY run_nginx /etc/sv/nginx/run
COPY run_waitress /etc/sv/waitress/run
RUN chmod +x /etc/sv/nginx/run
RUN chmod +x /etc/sv/waitress/run
